...
load_libs <- c('data.table','sf','ggplot2','grid','gridExtra','glue','raster','fasterize')
invisible(lapply(load_libs, library, character.only=TRUE))

# Settings
data_version <- 'DEFINE'
prev_data_version <- 'DEFINE'
...
# Load spatial data
uga <- sf::st_read(file.path(work_dir, 'viz/plotting_shps/districts_with_lakes.shp'))
outline_sf <- sf::st_read(file.path(work_dir, 'viz/plotting_shps/country_outline.shp'))

# Load input data
prev_data <- fread(file.path(prepped_dir, 'prev_data.csv'))
prev_data[, raw_prev := tb_cases / n ]
notif_data <- fread(file.path(prepped_dir, 'notif_data.csv'))
notif_data[, rate_adj := notif_adj / pop_adj ]
...
prev_sf <- merge(
  x=uga[, c('uid')],
  y=prev_data[, .(uid, district, tb_cases, n)],
  all.x=T
)
...
setnames(comp_summ, measures, paste0('comp_',measures))
prev_sf <- merge(
  x=prev_sf, y=comp_summ[ year==2019, .(uid,comp_mean,comp_lower,comp_upper)], all.x=TRUE
)
prev_sf$raw_prev <- prev_sf$tb_cases / prev_sf$n
# Set prevalence to rates per 100,000
for(prev_col in grep('prev', colnames(prev_sf), value=T)){
...
if(exists('prev_summ_no_notifs')){
  ...
  for(psnn_col in psnn_cols){
    prev_sf[[psnn_col]] <- prev_sf[[psnn_col]] * 1E5
  }
}
...
## FIG: INPUT DATA PLOT - CASE NOTIFICATIONS IN 2017 AND 2019
...
cn_data_sf <- merge(
  x = cn_data_sf,
  y = notif_data[, .(uid, district, year, rate_adj)],
  by = c('uid', 'year'),
  all.x = TRUE
)
cn_data_sf$rate_adj <- cn_data_sf$rate_adj * 1E5

fig_cn_data <- ggplot() +
  facet_wrap('year', nrow=1) +
  geom_sf(data=cn_data_sf, aes(fill=rate_adj), lwd=0.25, color='#222222') +
  country_outline +
  scale_fill_gradientn(
...
png(file.path(viz_dir, 'data_case_notifs.png'), height=3.5, width=7.5, units='in', res=300)
plot(fig_cn_data)
dev.off()
...
main_prev_a <- ggplot() +
  ...
  labs(
    title='A. Modeled TB Prevalence (Mean Estimate)', fill='TB prevalence\nper 100,000',
    x=NULL, y=NULL
  ) +
  theme_minimal() +
  theme(
...
main_prev_b <- ggplot(
  ...
  geom_point(size=2, aes(fill=tb_cases), shape=21) +
  ...
  labs(
    title='B. Comparison to TB Prevalence Survey',
    fill='Number of\nobserved\nTB cases',
...
notif_scatter_dt <- merge(
  x = prev_summ[, .(uid, prev_mean, prev_lower, prev_upper)],
  y = notif_data[, lapply(.SD, mean), by=uid, .SDcols = c('notif_adj', 'pop_adj')],
  by = 'uid'
)
notif_scatter_dt[, cnr_adj := notif_adj / pop_adj * 1E5 ]
notif_scatter_dt[, `:=` (
  prev_mean=prev_mean*1E5, prev_lower=prev_lower*1E5, prev_upper=prev_upper*1E5
)]
notif_scatter_dt[prev_upper >= 1500, prev_upper := 1500 - 1E-5]

n_breaks <- seq(0, 1000, by=200)
n_labs <- c(as.character(n_breaks[1:5]), '1,000+')
main_prev_c <- ggplot(
  data=na.omit(notif_scatter_dt),
  aes(x=cnr_adj, y=prev_mean, ymin=prev_lower, ymax=prev_upper)
) +
  geom_abline(intercept=0, slope=1, linetype=2, lwd=.5, color='#888888') +
  geom_crossbar(lwd=.25, color='#AAAAAA') +
  geom_point(size=1.5, aes(fill=notif_adj), shape=21, alpha=.5) +
  viridis::scale_fill_viridis(
    limits = range(n_breaks), breaks=n_breaks, labels=n_labs, oob = scales::squish
  ) +
  scale_x_continuous(oob=scales::squish, limits=c(0, 1500), labels=scales::comma) +
  scale_y_continuous(oob=scales::squish, limits=c(0, 1500), labels=scales::comma) +
  labs(
    title='C. Comparison to TB Case Notification Rate, 2015-2019',
    fill='Annual\ncase\nnotifications',
    x='Average CNR for pulmonary TB, ages 15+', y='Modeled TB prevalence'
  ) +
  theme_bw()

png(file.path(viz_dir, 'prev_scatter.png'), height=9, width=6, units='in', res=300)
grid.arrange(
  ggplotGrob(main_prev_a), ggplotGrob(main_prev_b), ggplotGrob(main_prev_c),
  layout_matrix=matrix(1:3, ncol=1)
)
dev.off()


## Plot: Completeness in 2017 and 2019
comp_sf <- merge(x=uga[, c('uid')], y=comp_summ, by='uid')
...
png(file.path(viz_dir, 'notif_completeness.png'), height=3.5, width=7.5, units='in', res=300)
plot(comp_fig)
dev.off()


## Plot: Average duration in 2017 and 2019

comp_sf$duration <- duration_intercept - duration_slope * comp_sf$comp_mean
...
ip_ratio_dt <- merge(
  x = prev_data[, .(uid, tb_cases, n)],
  y = notif_data[, .(uid, year, notif_adj, pop_adj)]
)
ip_ratio_dt <- merge(
  x = ip_ratio_dt,
  y = comp_summ[, .(uid, year, comp_mean, comp_lower, comp_upper)],
  by = c('uid','year')
)
ip_ratio_dt[, inc_prev_ratio := (notif_adj/pop_adj)/(tb_cases/n) * .82 ]
